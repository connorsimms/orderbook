...
public:
    ...
    bool canFullyFill(Price const& aggressorPrice, Size volumeNeeded) const
    {
        ...
        {
            if (comp_(aggressorPrice, level.price_)) break;

            for (const auto& resting: level.orders_)
            {
                if (resting->getOrderType() == OrderType::AllOrNone)
                {
                    if (resting->getRemainingSize() > volumeNeeded) continue;
                }
    ...
    Trades match(OrderId const& orderId, Side const& side, Price const& price, Size& volumeRemaining, const auto& onRemove)
    {
        ...
        {
            if (price != MARKET_PRICE && comp_(price, lvl->first)) break;
...
public:
    ...
    Trades match(OrderId const& orderId, Side const& side, Price const& price, Size& volumeRemaining, const auto& onRemove)
    {
        ...
        for (auto level = levels_.rbegin(); level != levels_.rend() && volumeRemaining > 0; )
        {
            if (price != MARKET_PRICE && comp_(price, level->price_)) break;
    ...
    void add(OrderPointer order)
    {
        ...
        auto it = std::lower_bound(rbegin(levels_), rend(levels_), orderPrice, 
    ...
    void cancel (OrderPointer order)
    {
        ...
        auto lvl = std::lower_bound(begin(levels_), end(levels_), orderPrice,
            [&comp](const PriceLevel<OrderContainer>& level, Price price) {
                return comp(level.price_, price);
            }
        );
...
public:
    ...
    Trades match(OrderId const& orderId, Side const& side, Price const& price, Size& volumeRemaining, const auto& onRemove)
    {
        ...
        for (auto level = levels_.begin(); level != levels_.end() && volumeRemaining > 0; )
        {
            if (price != MARKET_PRICE && comp_(price, level->price_)) break;
