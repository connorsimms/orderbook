...
{
    auto trades1 = this->orderbook_.addOrder(OrderType::GoodTillCancel, OrderId{1}, Side::Sell, Price{100}, Size{100});
    EXPECT_TRUE(trades1.empty());

    auto trades2 = this->orderbook_.addOrder(OrderType::GoodTillCancel, OrderId{2}, Side::Buy, Price{100}, Size{50});
    ...
    auto trades3 = this->orderbook_.addOrder(OrderType::GoodTillCancel, OrderId{3}, Side::Buy, Price{100}, Size{60});
    
    ASSERT_EQ(trades3.size(), 1);
    EXPECT_EQ(trades3[0].getBid().size_, 50); 
    EXPECT_EQ(trades3[0].getAsk().orderId_, 1);

    auto trades4 = this->orderbook_.addOrder(OrderType::GoodTillCancel, OrderId{4}, Side::Sell, Price{100}, Size{10});
...
{
    this->orderbook_.addOrder(OrderType::GoodTillCancel, OrderId{1}, Side::Sell, Price{101}, Size{10});
    this->orderbook_.addOrder(OrderType::GoodTillCancel, OrderId{2}, Side::Sell, Price{100}, Size{10}); // Better price
    this->orderbook_.addOrder(OrderType::GoodTillCancel, OrderId{3}, Side::Sell, Price{102}, Size{10});

    auto trades = this->orderbook_.addOrder(OrderType::GoodTillCancel, OrderId{4}, Side::Buy, Price{103}, Size{30});
...
TYPED_TEST(OrderBookTest, TimePriority) 
{
    ...
    this->orderbook_.addOrder(OrderType::GoodTillCancel, OrderId{2}, Side::Buy, Price{100}, Size{10});

    auto trades = this->orderbook_.addOrder(OrderType::GoodTillCancel, OrderId{3}, Side::Sell, Price{100}, Size{10});
